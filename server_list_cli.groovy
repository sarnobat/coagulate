...
public class CoagulateListCLI {
	...
	private static class RecursiveLimitByTotal2 {
		...
		static JsonObject getDirectoryHierarchies(String iDirectoryPathsString,
		...
		private static JsonValue createFilesJsonRecursiveNew(
				String[] iDirectoryPaths, int iLimit, Integer iDepth) {
			...
			for (String dirPath : merged.keySet()) {
				...
				JsonObject json2 = new SubDirObj(
						RecursiveLimitByTotal2
								.jsonFromString(RecursiveLimitByTotal2
										.createSubdirObjs(dirPath).toString()))
						.json();
		...
		private static Multimap<String, DirObj> toMultiMap(
				Collection<DirPair> allDirsAccumulated) {
		...
		private static Map<String, DirObj> mergeHierarhcies(
				Multimap<String, DirObj> unmerged) {
		...
		private static DirObj mergeDirsFold(DirObj dirObj, List<DirObj> dirObjs) {
			if (dirObjs.size() == 0) {
				...
				DirObj accumulatedSoFar = mergeDirectoryHierarchiesInternal(
						dirObj, dirObjs.get(0));
				return mergeDirsFold(accumulatedSoFar,
						dirObjs.subList(1, dirObjs.size()));
			}
		}
		...
		private static Set<String> getFilePaths(
				Collection<FileObj> filesAlreadyAdded) {
			System.err
					.println("Coagulate.RecursiveLimitByTotal2.getFilePaths() 1 ");
			Set<String> s = new HashSet<String>();
			for (FileObj f : filesAlreadyAdded) {
				String fileAbsolutePath = f.getFileAbsolutePath();
				System.err
						.println("Coagulate.RecursiveLimitByTotal2.getFilePaths() 2 "
								+ fileAbsolutePath);
				if (fileAbsolutePath == null) {
					// TODO: fix this
					System.err
							.println("Coagulate.RecursiveLimitByTotal2.getFilePaths() fileAbsolutePath = "
									+ f.json());
				} else {
			...
			System.err
					.println("Coagulate.RecursiveLimitByTotal2.getFilePaths() 5");
			return ImmutableSet.copyOf(s);
		}

		private static Set<FileObj> getFiles(
				Collection<DirPair> allDirsAccumulated) {
		...
		private static Set<Path> getSubPaths(Path iDirectoryPath,
				Filter<Path> isfile2) throws IOException {
			DirectoryStream<Path> filesInDir2 = Files.newDirectoryStream(
					iDirectoryPath, isfile2);
			Set<Path> filesInDir = FluentIterable.from(filesInDir2)
					.filter(SHOULD_DIP_INTO).toSet();
			filesInDir2.close();
			return filesInDir;
		}
		...
		private static final Predicate<Path> SHOULD_DIP_INTO = new Predicate<Path>() {
			...
			public boolean apply(Path input) {
				...
				return !forbidden.contains(input.getName(
						input.getNameCount() - 1).toString());
			}
		};

		private static DirObj mergeDirectoryHierarchiesInternal(DirObj dir1,
				DirObj dir2) {
			if (!dir1.getPath().equals(dir2.getPath())) {
				throw new RuntimeException(
						"Must merge on a per-directory basis");
			}
			String commonDirPath = dir1.getPath();
			Map<String, FileObj> files = mergeLeafNodes(dir1.getFiles(),
					dir2.getFiles());
			Map<String, DirObj> dirs = mergeOverlappingDirNodes(dir1.getDirs(),
					dir2.getDirs(), commonDirPath);
		...
		private static Map<String, DirObj> mergeOverlappingDirNodes(
				Map<String, DirObj> dirs1, Map<String, DirObj> dirs2,
			...
			for (String dirPath : Sets.union(dirs1.keySet(), dirs2.keySet())) {
				if (dirs1.containsKey(dirPath) && dirs2.containsKey(dirPath)) {
					ret.put(dirPath,
							mergeDirectoryHierarchiesInternal(
									dirs1.get(dirPath), dirs2.get(dirPath)));
				} else if (dirs1.containsKey(dirPath)
						&& !dirs2.containsKey(dirPath)) {
		...
		private static <T> Map<String, T> mergeLeafNodes(
				Map<String, T> leafNodes, Map<String, T> leafNodes2) {
			ImmutableMap.Builder<String, T> putAll = ImmutableMap
					.<String, T> builder().putAll(leafNodes);
		...
		private static JsonObject jsonFromString(String string) {
			if (string.contains("ebm:[locati")) {
				System.err
						.println("Coagulate.RecursiveLimitByTotal2.jsonFromString() - "
								+ string);
		...
		private static class PathToDirPair implements Function<String, DirPair> {
			...
			PathToDirPair(Set<String> filesAlreadyObtained, int iDepth,
					int iLimit) {
				_filesAlreadyObtained = ImmutableSet
						.copyOf(filesAlreadyObtained);
				depth = iDepth;
				_limit = iLimit;
			}
		...
		private static class PathToDirObj implements Function<String, DirObj> {
			...
			PathToDirObj(Set<String> filesAlreadyObtained, int iDepth,
					int iLimit) {
				_filesAbsolutePathsAlreadyObtained = ImmutableSet
						.copyOf(filesAlreadyObtained);
				depth = iDepth;
				_limit = iLimit;
			}
			...
			public DirObj apply(String dirPath) {
				JsonObject j;
				try {
					j = dipIntoDirRecursive(Paths.get(dirPath), 1,
							_filesAbsolutePathsAlreadyObtained, 0, _limit, 0,
							true, depth);
			...
			private static JsonObject dipIntoDirRecursive(Path iDirectoryPath,
					int filesPerLevel, Set<String> fileAbsolutePathsToIgnore,
			...
			private static Set<Path> getSubPaths(Path iDirectoryPath,
				...
				Set<Path> filesInDir;
				try {
					filesInDir2 = Files.newDirectoryStream(iDirectoryPath,
							isfile2);
					filesInDir = FluentIterable.from(filesInDir2)
							.filter(SHOULD_DIP_INTO).toSet();
			...
			private static ImmutableMap<String, JsonObject> getFilesInsideDir(
				...
				try {
					int addedCount = 0;
					Predicates.Contains predicate = new Predicates.Contains(
							filesToIgnore);
					for (Path p : FluentIterable
							.from(getSubPaths(iDirectoryPath,
		...
		private static class DirObj {
			...
			Map<String, FileObj> getFiles() {
				ImmutableMap.Builder<String, FileObj> ret = ImmutableMap
						.builder();
			...
			public Map<String, DirObj> getDirs() {
				ImmutableMap.Builder<String, DirObj> ret = ImmutableMap
						.builder();
				if (dirJson.containsKey("dirs")) {
					JsonObject dirs = dirJson.getJsonObject("dirs");
					for (String path : FluentIterable.from(dirs.keySet())
							.toSet()) {
		...
		private static class DirPair extends HashMap<String, DirObj> {
			...
			public JsonObject json() {
				return jsonFromString("{ \"" + dirPath + "\" : "
						+ dirObj.json().toString() + "}");
			}
	...
	private static class Predicates {
		...
		@Deprecated
		// We don't need a separate predicate
