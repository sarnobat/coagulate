...
String html = "<span id="+id+" style='" +style+ "'></span><script>var div = " +((char)36)+ "('#"+id+"'); var div2 = div;var jqxhr = "+dollar+ ".get('http://att.rohidekar.com:44452"							+ line							+ "', function(data) {	var i = 1;	var lines = data.split('\\n');	var out;	var titled = false;	for (var i = 0; i < lines.length; i++) {	var line = lines[i].replace(/^=+ /,'').replace(/ =+/,'');	if (!titled) {			if (/^[^=]/.test(line)) {		if (line.length < 40) {		out = '<h3>' +line + '</h3>'; }else { out = line; }				titled = true;				continue;			}		}		out += line + '<br>';	};	 "+dollar+"('#"+id+"').append(out);});</script>";
					output = html;
				} else {
					// program output
					output = line;
